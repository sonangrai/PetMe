 ...
 * @param res
 */
export const followProfileTask = async (req: Request, res: Response) => {
  try {
    //Checking if already followed
    let user = await Profile.findById(req.params.id);
    if (user.following.includes(req.params.uId)) {
      let newFollowing = await Profile.findOneAndUpdate(
        { _id: req.params.id },
        { $pull: { following: req.params.uId } },
        { new: true }
      );

      //Remove followers from another users followers
      await Profile.findOneAndUpdate(
        { _id: req.params.uId },
      ...
      let respObj = new ResponseObj(200, newFollowing, {}, "Unfollowed");
      return res.status(200).send(respObj);
    }

    //If not followed than we follow it
    let newFollowing = await Profile.findOneAndUpdate(
      { _id: req.params.id },
      { $push: { following: req.params.uId } },
      { new: true }
    );
    //Add followers to another users followers
    await Profile.findOneAndUpdate(
      { _id: req.params.uId },
      { $push: { followers: req.params.id } },
      { new: true }
    );
    let respObj = new ResponseObj(200, newFollowing, {}, "Followed");
    return res.status(200).send(respObj);
  } catch (error) {
    let errorObject: object = {};
    if (error instanceof Error) errorObject = error;
